    ...
    <div class="container">
        <div class="row gy-4">
            <div class="col-md-4">
                <a href="index.html" class="d-inline-block mb-2">
                    <img src="img/Logo/logo_1.png" alt="Logo de la marca" width="110" class="brand-logo-img">
                </a>
                <p class="small footer-store-desc">
                    <span class="store-name-display fw-bold">Syverluma Store</span> &mdash; <span class="store-slogan-display">Tu plataforma de comercio electrónico a la medida</span>.
                </p>
            </div>
            <div class="col-md-2">
                ...
                <ul class="list-unstyled">
                    ...
                    <li class="nav-item-blog"><a href="blog.html" class="text-white text-decoration-none">Blog</a></li>
                    <li class="nav-item-nosotros"><a href="nosotros.html" class="text-white text-decoration-none">Nosotros</a></li>
                    <li><a href="contactanos.html" class="text-white text-decoration-none">Contáctanos</a></li>
                </ul>
            </div>
            <div class="col-md-3 nav-item-servicios">
                <h6 class="text-uppercase fw-bold mb-3 footer-servicios-titulo">Servicios</h6>
                <ul class="list-unstyled footer-servicios-lista">
                    <li><a href="consultoria.html" class="text-white text-decoration-none">Consultoría</a></li>
                    <li><a href="soporte.html" class="text-white text-decoration-none">Atención y Soporte</a></li>
                </ul>
            </div>
            <div class="col-md-3">
                <h6 class="text-uppercase fw-bold mb-3">Contacto</h6>
                <ul class="list-unstyled small">
                    <li class="mb-1"><i class="bi bi-envelope me-1"></i> <a href="mailto:contacto@syverluma.com" class="text-white text-decoration-none footer-email">contacto@syverluma.com</a></li>
                    <li class="mb-1"><i class="bi bi-telephone me-1"></i> <a href="tel:+51916907657" class="text-white text-decoration-none footer-phone">+51 916 907 657</a></li>
                    <li class="mb-1"><i class="bi bi-geo-alt me-1"></i> <span class="footer-address">Lima, Perú</span></li>
                </ul>
                <div class="mt-3 footer-social-links">
                    <a href="#" target="_blank" rel="noopener noreferrer" class="text-white me-3 fs-5 footer-fb-link" title="Facebook"><i class="bi bi-facebook"></i></a>
                    <a href="#" target="_blank" rel="noopener noreferrer" class="text-white me-3 fs-5 footer-ig-link" title="Instagram"><i class="bi bi-instagram"></i></a>
                    <a href="#" target="_blank" rel="noopener noreferrer" class="text-white me-3 fs-5 footer-wa-link" title="WhatsApp"><i class="bi bi-whatsapp"></i></a>
                    <a href="#" target="_blank" rel="noopener noreferrer" class="text-white me-3 fs-5 footer-tk-link" title="TikTok"><i class="bi bi-tiktok"></i></a>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col text-center small">
                <span class="footer-copyright">&copy; 2026 Desarrollado por Corporación Syverluma S.A.C. &mdash; Todos los derechos reservados.</span>
...
function procesarPedidoWhatsApp() {
    ...
    let mensaje = `¡Hola eDark Import! 🐉 Quiero realizar el pedido *#${pedido.id}* con lo siguiente:\n\n`;
    carrito.forEach(item => {
        const p = Number(item.precio) || 0;
        const sub = p * Number(item.cantidad);
        mensaje += `• *${item.nombre}*\n  Cantidad: ${item.cantidad} x S/ ${p.toFixed(2)} = S/ ${sub.toFixed(2)}\n`;
    });
    mensaje += `\n*TOTAL A PAGAR: S/ ${totalSoles.toFixed(2)}*\n`;
    mensaje += `Método de envío/pago a coordinar. ¡Quedo atento!`;

    const targetPhone = (typeof phone !== 'undefined' && phone) ? phone : '51916907657';
    window.open(`https://wa.me/${targetPhone}?text=${encodeURIComponent(mensaje)}`, '_blank');
...
window.procesarPedidoWhatsApp = procesarPedidoWhatsApp;
...
function initPayPalButton() {
    ...
    if (paypalScriptEstado === 'deshabilitado' || (configGeneral && configGeneral.paypalHabilitado === false)) {
        document.querySelectorAll(containerSelector).forEach(c => {
            c.innerHTML = '';
...
async function cargarPayPalDesdeConfig() {
    ...
    try {
        ...
        if (cfg && cfg.paypalHabilitado === false) {
            ...
            const conts = document.querySelectorAll('#paypal-button-container');
            conts.forEach(cont => {
                cont.innerHTML = '';
...
function inicializarChimueloIA() {
    ...
    window.procesarConsultaChimuelo = function (texto, msgCont, inputEl) {
        ...
        setTimeout(async () => {
            ...
            // G) Búsqueda inteligente en Catálogo en Vivo (Productos, Hardware, Especificaciones)
            else {
                let cache = window.productosCache || [];
                if ((!cache || cache.length === 0) && typeof firebase !== 'undefined' && firebase && firebase.firestore) {
                    try {
                        const snap = await firebase.firestore().collection('productos').where('activo', '==', true).get();
                        cache = [];
                ...
                // Palabras vacías en español a filtrar del lenguaje natural
                const SPANISH_STOPWORDS = new Set([
                    'hola', 'buenos', 'dias', 'tardes', 'noches', 'buenas', 'saludos', 'estimado', 'amigo', 'amiga',
                    'quiero', 'quisiera', 'busco', 'buscando', 'necesito', 'necesitaria', 'tienen', 'tendran', 'tendras',
                    'venden', 'comprar', 'deseo', 'me', 'dame', 'dar', 'ver', 'mostrar', 'saber', 'dime', 'dinos',
                    'una', 'un', 'unos', 'unas', 'el', 'la', 'los', 'las', 'del', 'de', 'en', 'para', 'con', 'por',
                    'que', 'su', 'sus', 'mi', 'mis', 'favor', 'porfa', 'porfavor', 'precio', 'precios', 'cuanto',
                    'cuesta', 'valen', 'vale', 'hay', 'algun', 'alguna', 'alguno', 'algunos', 'producto', 'productos',
                    'articulo', 'articulos', 'stock', 'tienda', 'catalogo', 'disponible', 'disponibilidad'
                ]);

                // 1. Extraer palabras clave eliminando puntuación y palabras vacías
                const tokens = t.replace(/[^a-z0-9áéíóúñ\s]/gi, ' ').split(/\s+/).filter(Boolean);
                const palabrasClave = tokens.filter(tok => tok.length > 1 && !SPANISH_STOPWORDS.has(tok));

                function singularizar(w) {
                    let term = w.toLowerCase().trim();
                    if (term.length > 3) {
                        if (term.endsWith('es')) term = term.slice(0, -2);
                        else if (term.endsWith('s') && !term.endsWith('is') && !term.endsWith('ss')) term = term.slice(0, -1);
                    }
                    return term;
                }

                const terminos = palabrasClave.length > 0 ? palabrasClave : [t];
                const terminosSingulares = terminos.map(singularizar);

                // 2. Calcular puntuación de cada producto en el catálogo
                const productosPuntuados = cache.map(p => {
                    const nom = (p.nombre || '').toLowerCase();
                    const cat = (p.categoria || '').toLowerCase();
                    const sub = (p.subcategoria || '').toLowerCase();
                    const mar = (p.marca || '').toLowerCase();
                    ...
                    let score = 0;
                    // Coincidencia exacta de frase original
                    if (nom.includes(t) || cat.includes(t)) score += 100;

                    // Coincidencia por palabra clave extraída
                    terminos.forEach((term, idx) => {
                        const termSing = terminosSingulares[idx];

                        // Si coincide en Categoría o Subcategoría (ej: "tablet", "laptop", "monitor") -> Prioridad Alta (+40)
                        if (cat.includes(term) || cat.includes(termSing) || sub.includes(term) || sub.includes(termSing)) score += 40;

                        // Si coincide en Nombre (+30)
                        if (nom.includes(term) || nom.includes(termSing)) score += 30;

                        // Si coincide en Marca / Modelo (+25)
                        if (mar.includes(term) || mar.includes(termSing) || mod.includes(term) || mod.includes(termSing)) score += 25;

                        // Si coincide en Capacidad (+20)
                        if (cap.includes(term)) score += 20;

                        // Si coincide en Descripción / Tags (+15)
                        if (desc.includes(term) || desc.includes(termSing) || tags.includes(term) || tags.includes(termSing)) score += 15;
                    });
                ...
                const coincidencias = productosPuntuados.slice(0, 3).map(item => item.producto);
                const textoBusquedaLimpio = palabrasClave.length > 0 ? palabrasClave.join(' ') : texto;

                if (coincidencias.length > 0) {
                    respuestaHtml = `¡Encontré estas opciones disponibles para "<strong>${sanitize(textoBusquedaLimpio)}</strong>" en nuestro catálogo! 🐉🔥:<br><br>`;
                    ...
                    respuestaHtml = `
                        ¡Entendido! 🐉 No encontré un producto relacionado con "<strong>${sanitize(textoBusquedaLimpio)}</strong>" en el catálogo actual online, pero tenemos constantes ingresos en almacén y pedidos de importación a pedido.<br><br>
                        ¿Deseas consultar con un asesor de ventas por WhatsApp para verificar disponibilidad inmediata?
                        <div class="mt-2 text-center">
                            <a href="https://wa.me/51916907657?text=${encodeURIComponent('Hola, busco disponibilidad y precio de: ' + textoBusquedaLimpio)}" target="_blank" class="btn btn-sm btn-success w-100 fw-bold shadow-sm"><i class="bi bi-whatsapp me-1"></i>Consultar con un Asesor (+51 916 907 657)</a>
                        </div>`;
                }
            }
